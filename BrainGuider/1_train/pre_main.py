...
from pre_train import pre_train_model, test_pre_model
...
def result_print_and_save(metrics_list, save_dir, name):
    ...
    save_path = os.path.join(save_dir, f"pre_train_metrics_results_{name}.csv")
...
config_path = os.path.join(project_root, "1_train", "config", "base.yaml")

with open(config_path, "r") as f:
    config = yaml.safe_load(f)
    keys = ["train", "encoder_re", "decoder_re", "PredictorHead"]
    output = "\n".join(f"{key}: {config.get(key, '')}" for key in keys)


save_config_path = os.path.join(args.pre_model_save_dir, "config_used.yaml")
...
for i in range(5):
    ...
    best_model = pre_train_model(
        encoderModel=pre_encoderModel,
        decoderModel=pre_decoderModel,
        config=config,
        task_name=task_name,
...
result_print_and_save(metrics_list, args.pre_model_save_dir, "s100_h2")
